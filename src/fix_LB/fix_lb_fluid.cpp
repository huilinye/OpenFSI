...
//   check the force from the LAMMPS
     /* if(update->ntimestep % printfluid == 10){
      printf("     F_x          F_y          F_z\n");
	...
	for(k=0; k<3; k++){
	 // Ff[ixp][iyp][izp][k] += gammavalue*((v[i][k]*dt_lb/dx_lb)-unode[k])*FfP[isten];
	  Ff[ixp][iyp][izp][k] += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[isten];
	  if(fixviscouslb == 1 || groupbit_rigid_pc_sphere){
		  Ff[ixp][iyp][izp][k] += gammavalue*((v[i][k]*dt_lb/dx_lb)-unode[k])*FfP[isten];
	  }
	}
...
require more frequent neighborlist rebuilds");
  ...
  for(k=0; k<3; k++){
	...
	Ff[ix][iy][iz][k]    += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[0];
    Ff[ix][iy][izp][k]   += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[1];
    Ff[ix][iyp][iz][k]   += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[2];
    Ff[ix][iyp][izp][k]  += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[3];
    Ff[ixp][iy][iz][k]   += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[4];
    Ff[ixp][iy][izp][k]  += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[5];
    Ff[ixp][iyp][iz][k]  += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[6];
    Ff[ixp][iyp][izp][k] += 1.0*f[i][k]*dt_lb*dt_lb/dx_lb/dm_lb*FfP[7];
...
//Write the fluid properties(velocities, pressure) into VTK files.
//================================= By Huilin Ye.
    void FixLbFluid::Write_VTK_Fluid_File(void){
		
    /// Create filename

	stringstream output_filename;
	output_filename << "fluid_t" << update->ntimestep << ".vtk"; 
	ofstream output_file;
	...
	output_file << "DIMENSIONS " << Nbx << " " << Nby << " " << Nbz-2 << "\n";

	output_file << "X_COORDINATES " << Nbx << " float\n";
	...
	output_file << "\n";
	output_file << "Y_COORDINATES " << Nby  << " float\n";
	...
	output_file << "\n";
	output_file << "Z_COORDINATES " << Nbz-2 << " float\n";
	for (int Z = 1; Z < Nbz-1; ++Z) {
		output_file << Z*dx_lb << " ";
	}
	output_file << "POINT_DATA " << Nbx * Nby *(Nbz-2)  << "\n";
	...
	for (int Z = 1; Z < Nbz-1; ++Z) {
		for (int Y = 0; Y < Nby; ++Y) {
			for (int X = 0; X < Nbx; ++X) {
				output_file << altogether[X][Y][Z][0]*dm_lb/dx_lb/dx_lb/dx_lb << "\n";
			}
		}
	}
	...
	for (int Z = 1; Z < Nbz-1; ++Z) {
		for (int Y = 0; Y < Nby; ++Y) {
			for (int X = 0; X < Nbx; ++X) {
				output_file << altogether[X][Y][Z][1]*dx_lb/dt_lb <<" "<<altogether[X][Y][Z][2]*dx_lb/dt_lb <<" "<<altogether[X][Y][Z][3]*dx_lb/dt_lb << "\n";
			}
		}
	}
	/// Close file
