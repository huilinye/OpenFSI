...
/* ---------------------------------------------------------------------- */
 void FixLbPC::compute_up(void)
 {
  ...
  double ****volumeforce = fix_lb_fluid->Ff;
  double ***density_lb=fix_lb_fluid->density_lb;
  double bodyforcex = fix_lb_fluid->bodyforcex;
  double bodyforcey = fix_lb_fluid->bodyforcey;
  double bodyforcez = fix_lb_fluid->bodyforcez;
  double bodyforce[3];
  
  bodyforce[0]=bodyforcex;
  bodyforce[1]=bodyforcey;
  bodyforce[2]=bodyforcez;
   ...
   for(i=0; i<nlocal; i++){
	...
	for(ii=-1; ii<3; ii++){
	  ...
	  for(jj=-1; jj<3; jj++){
		...
		if(rsq>1){
	      ...
	      for(k=0; k<3; k++){
		up[i][k] += (u_lb[ixp][iyp][izp][k]+0.5*(volumeforce[ixp][iyp][izp][k]+bodyforce[k])/density_lb[ixp][iyp][izp])*FfP[isten];
	      }
	    }
	  }
	}
	...
	for (k=0; k<3; k++) { 	// tri-linearly interpolated velocity at node
	  up[i][k] = (u_lb[ix][iy][iz][k]+0.5*(volumeforce[ix][iy][iz][k]+bodyforce[k])/density_lb[ix][iy][iz])*FfP[0]
	    + (u_lb[ix][iy][izp][k]+0.5*(volumeforce[ix][iy][izp][k]+bodyforce[k])/density_lb[ix][iy][izp])*FfP[1]
	    + (u_lb[ix][iyp][iz][k]+0.5*(volumeforce[ix][iyp][iz][k]+bodyforce[k])/density_lb[ix][iyp][iz])*FfP[2]
	    + (u_lb[ix][iyp][izp][k]+0.5*(volumeforce[ix][iyp][izp][k]+bodyforce[k])/density_lb[ix][iyp][izp])*FfP[3]
	    + (u_lb[ixp][iy][iz][k]+0.5*(volumeforce[ixp][iy][iz][k]+bodyforce[k])/density_lb[ixp][iy][iz])*FfP[4]
	    + (u_lb[ixp][iy][izp][k]+0.5*(volumeforce[ixp][iy][izp][k]+bodyforce[k])/density_lb[ixp][iy][izp])*FfP[5]
	    + (u_lb[ixp][iyp][iz][k]+0.5*(volumeforce[ixp][iyp][iz][k]+bodyforce[k])/density_lb[ixp][iyp][iz])*FfP[6]
	    + (u_lb[ixp][iyp][izp][k]+0.5*(volumeforce[ixp][iyp][izp][k]+bodyforce[k])/density_lb[ixp][iyp][izp])*FfP[7];
	}
      }
      for(k=0; k<3; k++)
